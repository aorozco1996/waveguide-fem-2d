...
def plot_dispersion_curves(kc_sq_tm, kc_sq_te, a, b, waveguide_type):
    plt.figure()

    if waveguide_type == "rectangular":
        te_labels = ["TE10", "TE01/TE20", "TE11/TM11"]
        tm_labels = ["TE21/TM21", "TE31/TM31", "TE12/TM12"]
    ...
    if waveguide_type == "circular":
        te_labels = ["TE11", "TE21", "TE31"]
        tm_labels = ["TM01", "TM11", "TM21"]
    ...
    for i in range(6):
        ...
        ka_num = np.linspace(1.02 * ka_cutoff_num, ka_max, 8)
    ...
    plt.xlabel(r"$k a$")
    plt.ylabel(r"$\beta / k$")
    plt.title(f"Dispersion Curves for a {waveguide_type.capitalize()} Waveguide: Analytical vs FEM")
    plt.ylim(0, 1.05)
    plt.xlim(left=0)
    plt.grid(True)
    plt.legend()
...
def plot_mode_distribution(field, mode):
    pass